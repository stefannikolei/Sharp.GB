...
public static class RomTestUtils
{
    public static void TestRomWithMemory(string romPath)
    {
        ...
        MemoryTestRunner runner = new MemoryTestRunner(new(romPath), Console.Out);
    ...
    {
        Console.WriteLine("\n### Running test rom " + Path.GetFileName(romPath) + " ###");
        SerialTestRunner runner = new SerialTestRunner(new(romPath), Console.Out);
    ...
    {
        Console.WriteLine("\n### Running test rom " + Path.GetFileName(romPath) + " ###");
        MooneyeTestRunner runner = new MooneyeTestRunner(new(romPath), output);
